    ...
    def update_model(self, inputs, targets, tasks):
        ...
        for kappa in range(3):
            ...
            if self.task_count > 0:
                ...
                opt_buffer = torch.optim.SGD(cop.parameters(), lr=1e-05)
                J_PN_theta = self.criterion(self.forward(x_PN), y_PN, task_PN)
                for i in range(1):
                    opt_buffer.zero_grad()
                    self.criterion(cop.forward(x_PN), y_PN,
                                   task_PN).backward(retain_graph=True)
                    opt_buffer.step()
                J_th = (self.criterion(cop.forward(x_PN),
                             y_PN, task_PN) - J_PN_theta)
    ...
    def update_model(self, inputs, targets, tasks):
        ...
        for kappa in range(1):
            ...
            if self.task_count > 0:
                out = self.forward(inputs)
                Total_Loss = self.criterion(out, targets, tasks)
                index = torch.tensor(np.random.randint(0, xx.shape[0], 32))
                x_PN = xx[index]
                y_PN = yy[index]
                # print("The max target is", torch.max(y_PN) )
                tt_list = (tt_list+tasks)
                ...
                # ##############################################################
                cop = copy.deepcopy(self.model)
                opt_buffer = torch.optim.SGD(cop.parameters(),lr = 0.01) 
                J_PN_theta = self.criterion(self.forward(x_PN), y_PN, task_PN)
                for i in range(50):
                    opt_buffer.zero_grad()
                ...
                print("The four losses J_P, J_th, J_x", J_P.item(), J_th.item(), J_x.item(), Total_Loss.item() )
